...
void outputXPMFile(int lowerBoundX, int lowerBoundY, int upperBoundX, int upperBoundY, float zMin, float zMax)
{
	...
	for (Shape* s : shapes)
	{
		s->draw(zBuff, screen, height, width, zMax);
	...
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+36) << " c #" << std::hex << 255 - 12*i << "0000\"," << endl;
	}
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+56) << " c #00" << std::hex << 255 - 12*i << "00\"," << endl;
	}
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+76) << " c #0000" << std::hex << 255 - 12*i << "\"," << endl;
	}
