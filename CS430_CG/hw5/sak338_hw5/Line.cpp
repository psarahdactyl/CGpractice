...
void Line::draw(float ** zBuff, char **screen, int height, int width)
{
	...
	else
	{
		...
		*/
		...
		{
			x += dx;
			float pz = z1 - (z1 - z2) * (y1 - (float)y) / (y1 - y2);
			if (y < 0 || x < 0 || y >= height || x >= width)
				continue;
...
void Line::fill(float ** zBuff, char **screen, int height, int width, float zMin, float zMax)
{
}
