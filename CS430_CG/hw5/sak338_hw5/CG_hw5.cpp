...
void parseSMFFile(string name, int color)
{
	ifstream infile;
...
void outputXPMFile(int lowerBoundX, int lowerBoundY, int upperBoundX, int upperBoundY, float zMin, float zMax)
{
	int width = 501;
	int height = 501;
	...
	for (Shape* s : shapes)
	{
		//s->draw(zBuff, screen, height, width);
		s->fill(zBuff, screen, height, width, zMin, zMax);
		i++;
	}
	...
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+36) << " c #" << std::hex << 255 - 9*i << "0000\"," << endl;
	}
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+56) << " c #00" << std::hex << 255 - 9*i << "00\"," << endl;
	}
	for(int i=0; i < 20; i++)
	{
		cout << "\"" << (char)(i+76) << " c #0000" << std::hex << 255 - 9*i << "\"," << endl;
	}
...
int main( int argc, char **argv )
{
	...
	for(int i = 1; i < argc; i++)
	{
		...
		else if(flag.find("-g") != string::npos)
		{
			parseSMFFile(arg, g);
		}
		else if(flag.find("-i") != string::npos)
		{
			parseSMFFile(arg, b);
		}
	...
	{
		//default file
		string dFile = "bound-sprellpsd.smf";
		parseSMFFile(dFile, r);
	}
	...
	worldToViewport(-1, 1, -1, 1,
 					viewLowerBoundX, viewUpperBoundX, viewLowerBoundY, viewUpperBoundY); 	
	outputXPMFile(0,501,0,501,zFar, zNear);
