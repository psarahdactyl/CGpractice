...
// CS 432
// Assignment 9 part 2
// Assignment 6 with procedural texturing
...
// Texture
int texture;
GLuint my_texels[16][16][16][3];
double noise[16][16][16];
...
double smoothNoise(double x, double y, double z)
{
	...
	value += f_x * f_y * f_z * noise[x1][y1][z1];
	value += f_x * (1 - f_y) * (1 - f_z) * noise[x1][y2][z2];
	value += (1 - f_x) * f_y * (1 - f_z) * noise[x1][y1][z2];
	value += (1 - f_x) * (1 - f_y) * f_z * noise[x2][y2][z1];
	value += f_x * (1 - f_y) * f_z * noise[x1][y2][z1];
	value += (1 - f_x) * f_y * f_z * noise[x2][y1][z1];
	value += f_x * f_y * (1 - f_z) * noise[x1][y1][z2];
	value += (1 - f_x) * (1 - f_y) * (1 - f_z) * noise[x2][y2][z2];
...
void init( )
{
	...
	for ( int i = 0; i < 16; i++ ) 
	{
		for ( int j = 0; j < 16; j++ ) 
		{
			for ( int k = 0; k < 16; k++ ) 
			{
				double xyz = i * 50 / 16 + j * 10 / 16 + k * 6 / 16 + 90 * turbulence(i, j, k, 8) / 255.0;
				double marble = 255 * fabs(sin(xyz * M_PI));
				GLubyte c = (GLubyte)marble;
	...
	glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA, 16, 16, 16, 0, GL_RGBA, GL_UNSIGNED_BYTE, my_texels);
	...
	program = InitShader( "vshdr2.glsl", "fshdr2.glsl" );
...
int main( int argc, char **argv )
{
	...
	// Main Window
    mainWin = glutCreateWindow( "3D TEXTURE, YO." );
